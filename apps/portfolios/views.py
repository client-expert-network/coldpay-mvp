...
#     if request.method == "GET":
#         form = PortfolioForm()
#         return render(request, "portfolios/create_test.html", {"form": form})
...
# @require_http_methods(["GET", "POST"])  
# def detail_test(request, portfolio_id):    # 게시물 READ
#     # 특정 Portfolio 객체를 가져옴
#     portfolio = get_object_or_404(Portfolio, pk=portfolio_id)
...
#             request.session[session_key] = True
#         portfolio.save()
#         return HttpResponseRedirect(reverse('test_detail', args=[portfolio_id]))
